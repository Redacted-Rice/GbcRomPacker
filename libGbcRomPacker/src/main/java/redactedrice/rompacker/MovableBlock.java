...
public class MovableBlock extends AllocBlock {
    ...
    public MovableBlock(SegmentedByteBlock code, int priority, byte startBank,
            byte stopBankExclusive) {
        this(code);
    ...
    public void addAllowableBankRange(int priority, byte startBank, byte stopBankExclusive) {
        allowableBankPreferences
                .add(new PrioritizedBankRange(priority, startBank, stopBankExclusive));
    }
    ...
    byte popNextUnattemptedAllowableBank() {
        ...
        PrioritizedBankRange pref = new PrioritizedBankRange(
                unattemptedAllowableBankPreferences.first());
    ...
    private void removeUnattemptedBank(byte bank) {
        ...
        while (iter.hasNext()) {
            ...
            if (currPref.contains(bank)) {
                // We always start with the first one in the range so that makes things easier since
                // we don't have to worry about splitting banks
                iter.remove();
    ...
    public static class PrioritySorter implements Comparator<MovableBlock> {
        public int compare(MovableBlock a1, MovableBlock a2) {
            int compareVal = Integer.compare(a1.getNextUnatemptedAllowableBankPriority(),
                    a2.getNextUnatemptedAllowableBankPriority());

            if (compareVal == 0) {
                // Give larger blocks higher priority - We have to do it agnostic to where things
                // are
            ...
            if (compareVal == 0) {
                // if all else fails, use the unique block ID so things like sets will recognize
