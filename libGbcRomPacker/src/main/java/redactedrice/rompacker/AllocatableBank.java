...
public class AllocatableBank {
    ...
    public void addSpace(AddressRange space) {
        ...
        if (!bankRange.contains(space)) {
            throw new IllegalArgumentException("Passed space is not entirely in this bank (" + bank
                    + ")!" + " bank addresses: " + Arrays.toString(RomUtils.getBankBounds(bank))
                    + " and space addresses: " + space.getStart() + ", "
                    + space.getStopExclusive());
        }
    ...
    public void removeAddressSpace(AddressRange range) {
        ...
        if (bankRange.overlaps(range)) {
            ...
            // Then go through and remove the overlap potentially adding a new space in if the
            // removed space is in the middle
            ...
            while (spaceItr.hasNext()) {
                ...
                if (space.isEmpty()) {
                    ...
                    // We don't need to add the split now since we already have checked the space it
                    // came from
    ...
    private List<AddressRange> getSpacesLeftRemovingFixedAllocs(
        ...
        for (FixedBlock block : fixedAllocations) {
            if (!tryRemoveEntireContainingSpace(block, spacesLeft, assignedAddresses)) {
                throw new RuntimeException(String.format(
                        "Desync error! There was no longer space in bank 0x%x for FixedBlock %s from 0x%x to 0x%x - "
                                + "only ReplacementBlocks do not need free space in the bank",
                        bank, block.getId(), block.getFixedAddress().getAddressInBank(),
                        block.getFixedAddress().getAddressInBank()
                                + block.getWorstCaseSize(assignedAddresses)));
            }
        }
        AddressRange.sortAndCombine(spaces);
    ...
    private boolean tryRemoveEntireContainingSpace(FixedBlock block, List<AddressRange> spacesLeft,
            AssignedAddresses assignedAddresses) {
        AddressRange fixedRange = new AddressRange(block.getFixedAddress().getAddressInBank(),
                block.getFixedAddress().getAddressInBank()
                        + block.getWorstCaseSize(assignedAddresses));
    ...
    public boolean checkForAndRemoveExcessAllocs(List<MovableBlock> allocsThatDontFit,
            AssignedAddresses assignedAddresses) {
        ...
        return checkForAndRemoveExcessAllocsInCollection(priortizedAllocations.iterator(),
    ...
    private boolean checkForAndRemoveExcessAllocsInCollection(Iterator<MovableBlock> allocItr,
        ...
        while (!stable) {
            ...
            // Reassign the fixed block addresses in case it can shrink based on where the movable
    ...
    public void assignAddresses(AssignedAddresses assignedAddresses) {
        ...
        for (MovableBlock block : priortizedAllocations) {
            ...
            for (AddressRange space : spacesLeft) {
                if (allocSize <= space.size()) {
                    block.assignAddresses(new BankAddress(bank, (short) space.getStart()),
                            assignedAddresses);
                    space.shrink(allocSize);
                    placed = true;
                    break;
            ...
            if (!placed) {
                throw new RuntimeException(String.format("Failed to assign a address to block "
                        + block.getId()
