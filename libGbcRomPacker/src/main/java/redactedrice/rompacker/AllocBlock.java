...
public abstract class AllocBlock {
    ...
    public AllocBlock(SegmentedByteBlock code) {
        this.block = code;
        this.endSegmentName = SegmentNamingUtils.formSubsegmentName(END_OF_DATA_BLOCK_SUBSEG_LABEL,
                block.getId());
    }
    ...
    protected BankAddress assignCodeBlockAddress(BankAddress blockAddress,
            AssignedAddresses assignedAddresses) {
        if (!blockAddress.isFullAddress()) {
            throw new IllegalArgumentException("Attempted to assign an incomplete address ("
                    + blockAddress + ") to AllocBlock \"" + block.getId() + "\"");
        }

        // Get the relative addresses for the segments
        AssignedAddresses relAddresses = new AssignedAddresses();
        BankAddress relativeSegEnd = block.getSegmentsRelativeAddresses(blockAddress,
        ...
        while (segItr.hasNext()) {
            ...
            BankAddress relAddress = blockAddress.newSum(relAddresses.getThrow(segmentId),
                    BankAddressToUseType.ADDRESS_IN_BANK_ONLY,
                    BankAddressLimitType.WITHIN_BANK_OR_START_OF_NEXT);

            // If its null, we passed the bank. Otherwise if its the next bank, then we reached the
            // end perfectly so if this
            // was the last segment then we are good. Otherwise we are in trouble
            if (relAddress == null
                    || ((!relAddress.isSameBank(blockAddress)) && segItr.hasNext())) {
                throw new RuntimeException(
                        "assignBlockAndSegmentBankAddresses Passed the end of a bank "
                                + "while assigning addresses for segment (" + segmentId
                                + "). Each data block "
